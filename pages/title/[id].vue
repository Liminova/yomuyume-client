...
import { Action, indexApi, userApi, fileApiUrl } from "~/composables/api";
...
// Fetching all infos
void (async () => {
	const res = await indexApi.title(id, setSnackbarMessage);
...
function toggleFavorite() {
	void userApi
		.favorite(id, isFavorite.value ? Action.DELETE : Action.PUT, setSnackbarMessage)
		.then((ok) => {
			if (!ok) {
				return;
			}
...
function toggleBookmark() {
	void userApi
		.bookmark(id, isBookmark.value ? Action.DELETE : Action.PUT, setSnackbarMessage)
		.then((ok) => {
			if (!ok) {
				return;
			}
